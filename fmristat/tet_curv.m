...
%TET_CURV Calculates effective curvature of a tetrehedral mesh.
%
% TET_CURV( TET_FILE, OUTPUT_FILE_BASE [,DFW])
%
% TET_FILE: Tetrehedral mesh file from mesh_tet, see help of mesh_tet.
%
% OUTPUT_FILE_BASE_curv.img or .mnc is the effective curvature, mm^(-1).
%
...
%
% Curvature is based on the following calculation. If the manifold was
% part of a 4-sphere with radius r, curvature 1/r, i.e. coordinates
% (x,y,z,r)/sqrt(x^2+y^2+z^2+r^2)*r, then its intrinsic volumes are
% mu_1=3*pi*r, mu_3=2*pi^2*r^3. The roughness is sqrt(lam)=1/r, so the
% density of the 1D intrinsic volume * sqrt(lam) = mu_1 * sqrt(lam) / mu_3   
% = 3/(2*pi) / r^3. Equating this to the observed density gives the
% effective curvature, 1/r. The integral of the curvature^3 * 3/(2*pi) 
% is the contribution from the interior of the search region to the 1D 
% intrinsic volume. 
...
ext=tet_file(1,(size(tet_file,2)-2):size(tet_file,2));
curv.file_name=[deblank(output_file_base(1,:)) '_curv.' ext];
curv.dim=[I J numslices 0];
curv.parent_file=tet_file(1,:);

f=(2*pi/3/abs(prod(d.vox))/biasr(2))^(1/3);
