...
if ischar(d)
   ...
   input_file=[d.file_path d.file_name];
   isfile=0;
end

if isempty(output_file_base)
   output_file_base=input_file(1:(min(findstr('.',input_file))-1));
end
...
if frames>1
   fwrite(fid,length(frames),'float');
end
...
if isfile
   ...
   if length(frames)==1
      ...
      fwrite(fid,permute(d.data(x,y,z,:),[4 1 2 3]),'float');
   end
end
fclose(fid)
