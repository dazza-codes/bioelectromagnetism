function [V, D]=pca_image(input_file, exclude, p, ...
    mask_file, mask_thresh, output_file_base, X_remove, X_interest)
...
%         [, OUTPUT_FILE_BASE [, X_REMOVE [, X_INTEREST ]]]]]]])
%
% INPUT_FILE is a matrix of image file names, padded with blanks if  
% necessary, in ANALYZE (.img) or MINC format (.mnc) or both.
%
...
for i=1:numfiles
   d=fmris_read_image(deblank(input_file(i,:)),0,0);
   numframes(i)=d.dim(4);
end
if numfiles<=5
   numframes
...
numxs=d.dim(1)
numpix=numxs*numys;
ext=input_file(1,min(findstr('.',input_file(1,:)))+(1:3));
...
for slice=1:numslices
   ...
   for i=1:numfiles
      ...
      d=fmris_read_image(deblank(input_file(i,:)),slice,keepin);
...
zlim=[min(bigmat(:)) max(bigmat(:))]*0.8;
