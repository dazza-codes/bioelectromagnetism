function view_slices(input_file, mask_file, mask_thresh, slices, frames, zlim)
...
xp=xr-1; signx=' '; if (isminc & d.vox(1)<0) | isanal; xp=-(d.dim(1)-xp-1); signx='-'; end
...
if nslices>1
   ...
   axis xy; set(gca,'XTick',[]); set(gca,'YTick',[]); 
   if length(slices)>1
      title([strrep(input_file,'_',' ') ', frame ' num2str(frames)]);  
   end
   if length(frames)>1
      title([strrep(input_file,'_',' ') ', slice ' num2str(slices)]);  
   end
