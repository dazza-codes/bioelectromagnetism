...
if nargin < 6
   shift=[-4.5 4.5]*max(hrf_parameters(2)/5.2,1);
end
...
for k=1:numresponses
   ...
   % svd:
   [U,S,V]=svd(H,0);
   prcnt_var_spectral=(S(1,1)^2+S(2,2)^2)/sum(diag(S).^2)*100;
   sumU=sum(U(:,1));
   US=U(:,1:2)/sumU;
   WS=V(:,1:2)*S(1:2,1:2)*sumU;
   if delta*WS(:,2)<0
      US(:,2)=-US(:,2);
      WS(:,2)=-WS(:,2);
   end
   ...
   if ~all(WS(:,1)>0)
      fprintf(['Warning: first coef not positive for stimulus ' num2str(k)]);
   end
   cubic_coef=pinv([delta' delta'.^3])*(WS(:,2)./WS(:,1));
   if prod(cubic_coef)<0
      fprintf(['Warning: svd ratio not invertible for stimulus ' num2str(k)]);
   end
end 
