...
machineformat=getmachineformat(deblank(file));

if ~isempty(machineformat)
   
   % Read Header Information
   d=Read_Analyze_Hdr(deblank(file),machineformat);
   ...
   if ~isempty(d) & ~strcmp(d.precision,'Unknown') 
      ...
      if nargin==1 | strcmp(d.precision,'uint1') 
         ...
         fid = fopen([d.file_path d.file_name '.img'],'r',machineformat);
         if fid > -1
         ...
         if nargin==3
            if prod(Z>0)&prod(Z<=d.dim(3))&prod(T>0)&prod(T<=d.dim(4))
               d.data=d.data(:,:,Z,T);
               d.Z=Z;
               d.T=T;
            else
...
file_l    		= length(file);
if file(file_l-3)=='.';
   file=file(1:(file_l-4));
   file_l = length(file);
end;
i=max([findstr(file,'/') findstr(file,'\')]);
d.file_path		= file(1:i);
if isempty(d.file_path); 
   d.file_path	= './';
   d.file_name	= file;
else
   d.file_name	= file((i+1):file_l);
end

machineformat=[];
for mf='nlbdgcas'
   fid = fopen([d.file_path d.file_name '.hdr'],'r',mf);
   if fid > -1
      fseek(fid,40,'bof');
...
h=dir([d.file_path d.file_name '.img']);
...
fid  = fopen([d.file_path d.file_name '.hdr'],'r',machineformat);
...
machineformat='n';
file='c:/keith/fMRI/cohen/4A50SJS1.IMG';
dim=[64 64 6 108];
range=[0 956];
