function [z,zprime]=bvp2(f,t,x0,bvpopt,parvec)
%   BVP2 Solve second order boundary value problem
%
%   [X,XPRIME]=BVP(F,T,X0[,OPTIONS,PARVEC]) solves the boundary
%   value problem
%
%      x"=f(t,x,x') , t \in [a,b] , R(x(a),x'(a),x(b),x'(b))=0
%
%   using a generalisation of the symmetric difference discretisation
%   to nonequidistant grids (local discretisation error=O(h^2)).
%
%    h2/h*x1 -x2 -h1/h*x3 + (h1*h2)/2*f((t1+t3)/2,(x1+x3)/2,(x3-x1)/h)
%
%   where h=h1+h2. Due to use of additional points at the ends of the 
...
%   the BVP2_File_Pattern at the end of this text.
%   
...
%   ***************************************************************
%
%   The output X should be a solution. X and XPRIME have the same 
%   form as X0.
%
%   ***************************************************************
%
%   function out=BVP2_File_Pattern(t,x,xprime,xN,xNprime,flag[,parvec])
%
...
%     out=<df/dx'(t,x,x')>;
%   case 'R' 
%     x1=x;             %   x1 and x1' are passed;
%     x1prime=xprime;   %   as 2nd and 3rd argument
%     out=<R(x1,x1',xN,xN')>;
%   case 'dR/dx1'   
%     x1=x;             
%     x1prime=xprime;
%     out=<dR/dx1(x1,x1',xN,xN')>;
%   case 'dR/dx1prime'   
%     x1=x;             
%     x1prime=xprime;
%     out=<dR/dx1'(x1,x1',xN,xN')>;
...
%   end
%
%   See also BVP  BVPSET  BVPPLOT  BVPPHAS2  BVPPHAS3
...
teff=t;                  % t_effective
t=[t(1)-(t(2)-t(1)) ;  t ; t(end)+(t(end)-t(end-1))];  % additional variables
x0=[x0(1,:) ; x0 ; x0(end,:)];                         % for O(h^2) on the boundary
...
if isempty(bvpopt.outputfcn)
   x=sparsefsolve('bvp2_disc',x,bvpopt.fsolve,'bvp2_disc_jac',f,t,dim,N,h,bvpopt,parvec);
else
...
case 1
   z=zeros(dim,N-2);
   z(:)=x(dim+1:N*dim-dim);   % wieder Matrix-Indizierung und raus damit
   z=z';
case 2
   z=zeros(dim,N);
   zprime=zeros(N-2,dim);
   
   z(:)=x;                    % wieder Matrix-Indizierung und raus damit
   z=z';
   
   for i=1:N-2
      zprime(i,:)=(z(i+2,:)-z(i,:))/(h(i)+h(i+1));
   end
