...
% $Revision: 1.4 $ $Date: 2007/02/16 01:24:26 $
...
if isempty(A),
  error('Input matrix A is empty\n');
end
if isempty(B),
  error('Input matrix B is empty\n');
end
...
if ~isequal(As(2),3),
  if isequal(As(1),3),
    ...
    error('Input matrix A should be Nx3 [x y z] coordinates\n');
  end
end
...
if ~isequal(Bs(2),3),
  if isequal(Bs(1),3),
    ...
    error('Input matrix B should be Nx3 [x y z] coordinates\n');
  end
end
