...
if nargin < 2
	...
	xd = dtft2_adj(X(:), [o1(:) o2(:)], N1, N2);
	xl = dtft2_adj(X(:), [o1(:) o2(:)], N1, N2, [], 1);
	disp(sprintf('loop max %% difference = %g', max_percent_diff(xl,xd)))
	xf = col(ifft2(X)) * N1 * N2;
	disp(sprintf('ifft max %% difference = %g', max_percent_diff(xf,xd)))
	return
end
...
if useloop
	...
	x = exp(i*(nn1(:)*omega(:,1)'+nn2(:)*omega(:,2)')) * X;	% [N1*N2,L]
