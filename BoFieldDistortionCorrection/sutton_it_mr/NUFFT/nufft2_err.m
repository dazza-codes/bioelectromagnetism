 function [err,f1,f2] = nufft1_err(om, N1, N2, J1, J2, K1, K2, alpha, beta)
%function err = nufft1_err(om, N1, N2, J1, J2, K1, K2, alpha, beta)
%	Compute worst-case error for each input frequency for 2D NUFFT.
%	in:
...
if nargin < 4
	...
	alpha = 1;
	alpha = 'best';
	[err,f1,f2] = nufft2_err('all', N1, N2, J1, J2, K1, K2, alpha);
	mesh(f1, f2, err)
	plot(f1, err)
...
%
if ischar(om)
	if ~strcmp(om, 'all'), error 'unknown om argument', end
	gam1 = 2*pi/K1;
	gam2 = 2*pi/K2;
...
tol = 0;
R1 = nufft_R(N1, J1, K1, tol, alpha1, beta1);		% [J,J]
R2 = nufft_R(N2, J2, K2, tol, alpha2, beta2);		% [J,J]
r1 = nufft_r(om(:,1), N1, J1, K1, alpha1, beta1);	% [J,M]
r2 = nufft_r(om(:,2), N2, J2, K2, alpha2, beta2);	% [J,M]
R = kron(R2, R1);	% J1*J2 x J1*J2
