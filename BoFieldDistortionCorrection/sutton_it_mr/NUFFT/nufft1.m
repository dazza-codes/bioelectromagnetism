...
if nargin < 1
	...
	Sprintf('initialization time %g', toc)
	[Xn, times] = nufft1(x, st);
	if 1	% Kaiser-Bessel
		sk = nufft1_init_kb(o1, N1, J1, K1, n_shift, 'kaiser');
		Xk = nufft1(x, sk);
		Sprintf('kb max %% difference = %g', max_percent_diff(Xd,Xk))
	end
	if 1
	...
	if 1	% min-max with KB scaling
		...
		Sprintf('mmkb max %% difference = %g', max_percent_diff(Xd,Xmmkb))
	end
	Sprintf('unif max %% difference = %g', max_percent_diff(Xd,Xn))
	...
	legend('DTFT r', 'DTFT i', ...
		'KB r', 'KB i', ...
		'NUFFT r', 'NUFFT i')
	Sprintf('time fft=%g other=%g', times(1), times(2))
return
end
