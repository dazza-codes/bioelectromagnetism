...
%	x = zeros(N1,N2); x(1,1) = 1;
	...
	Sprintf('kb loop max %% difference = %g', max_percent_diff(sk.p,s1.p))
	...
	Sprintf('mm loop max %% difference = %g', max_percent_diff(sm.p,s1.p))

	Xm = nufft2(x, sm);
	Sprintf('alf1 max %% difference = %g', max_percent_diff(Xd,Xm))
