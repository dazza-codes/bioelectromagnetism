...
%
%   Parameters can be passed as a structure (p).  A standard set of
%   parameters can be created using 'eeg_toolbox_defaults'. See the .m file
%   of that function for some information about the parameters created for
%   contours.  As of writing help for this function, it contains the
%   following parameters related to 'eeg_contours_engine':
%
%    p.rangeMethod                 = 'minmaxabs' ;
%                                     minmax? = 'abs','one','all','giv'
%    p.minimumIntensity            =  -10    ;	
%    p.maximumIntensity            =   10    ;
%    p.interpMethod                = 'cubic' ;   
%                                     matlab griddata interpolation method
%    p.timeMethod                  =    1    ;
%                                       1=single time point,
%                                       2=range of time points
%    p.endTime                     =    2    ;   
%                                       finish time point (a row of p.volt.data)
%    
%    p.contour.stepMethod          =  0; % 0 is step size, 1 is number of steps
%    p.contour.Nsteps              = 10; % or length of p.colorMap.map
...
%
%  This is all subject to change without notice. Sorry, it's difficult
...
% $Revision: 1.5 $ $Date: 2007/03/15 04:25:27 $
...
if p.mesh.plotSurf,
  ...
  if isempty(p.mesh.current),
    ...
    if ~isequal(p.volt.data',TMP(1:Nelec,:)),
      ...
      msg = sprintf('scalp time series ~= electrode time series - recalculating.\n');
      warning(msg);
...
if or(p.elec.plotSurf, p.mesh.plotSurf),
  ...
  if p.elec.plotSurf,
    ...
    p.elec.patch = patch('vertices',[Xp,Yp,Zp],...
                         'faces',convhulln([Xp,Yp,Zp]),...
                         'facevertexCdata',V,...
                         'facecolor','interp',...
                         'EdgeColor','none',...
                         'FaceLighting','phong');
  ...
  while (i < limit),
    ...
    if (contourValues(j) < 0)
      ...
      if isequal(p.colorMap.style,'Gray')
        line(X3,Y3,Z3,...
             'LineStyle',':',...
             'LineWidth',p.contour.lineWidth,...
             'Color',p.contour.lineColor);
      else
        line(X3,Y3,Z3,...
             'LineStyle',':',...
             'LineWidth',p.contour.lineWidth,...
    ...
    elseif (contourValues(j) > 0)
      ...
      % Values = 0 with thicker solid line
      line(X3,Y3,Z3,...
           'LineStyle','-',...
           'LineWidth', p.contour.lineWidth + 0.25,...
           'Color',p.contour.lineColor);
    end
    i = endContourSet + 1;
  end
