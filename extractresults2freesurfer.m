...
% cd E:\matlab\brainstorm_v1\studies; extractresults2freesurfer
...
for g=1:length(groups),
    for s=1:length(subs),
        ...
        % temporarily skip subject c04
        if strmatch(sub,'c04'), continue; end
        ...
        for c = 1:length(cond),
            ...
            case 'sa',
                ...
                for t=1:length(sa),
                    ...
                    if length(col) > 1, error('> 1 matching time column in ImageGridTime\n'); end
                    
                    p.mesh.data.Cdata{p.mesh.current} = abs(ImageGridAmp(:,col) .* scale);
                ...
                for t=1:length(sa),
                    ...
                    p.mesh.data.Cdata{p.mesh.current} = abs(Dif.oac(:,t) - Dif.ouc(:,t));
            ...
            case 'wm',
                ...
                for t=1:length(wm),
                    ...
                    p.mesh.data.Cdata{p.mesh.current} = abs(Dif.tac(:,t) - Dif.oac(:,t));
            ...
            case 'ea',
                ...
                for t=1:length(ea),
                    ...
                    p.mesh.data.Cdata{p.mesh.current} = abs(Dif.oat(:,t) - Dif.oac(:,t));
