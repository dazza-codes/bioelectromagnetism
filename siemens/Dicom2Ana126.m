...
if (isempty(strmatch('Image' , fieldnames(Patient{patient_no(1)}.Study{study(1)}.Series{series(1)}))))
    ...
    if (strcmp(Patient{patient_no(1)}.Study{study(1)}.Series{series(1)}.Whole_Structure.ImageType , 'ORIGINAL\PRIMARY\M\MOSAIC') | ...
            strcmp(Patient{patient_no(1)}.Study{study(1)}.Series{series(1)}.Whole_Structure.ImageType , 'ORIGINAL\PRIMARY\M\ND\MOSAIC') | ...
            ~isempty(strfind( char(Patient{patient_no(1)}.Study{study(1)}.Series{series(1)}.Whole_Structure.Private_0029_1020)' , '<ParamFunctor."MosaicUnwrapper">')))
            ...
            for k = series
                ...
                for l = image
                    ...
                    if ( isstruct (Patient{i}.Study{j}.Series{k}.Image{l}) == 1)
                        ...
                        if (isempty(strfind(Patient{i}.Study{j}.Series{k}.Whole_Structure.ProtocolName,lower(lategd_filename))) & isempty(strfind(Patient{i}.Study{j}.Series{k}.Whole_Structure.ProtocolName,upper(lategd_filename))))
                            % This is not a Tissue Phase Map file so carry on as normal           
                        else
                            if (lategd_file == 1)
                                %The Freiburg TPM code requires that '.dcm' is the suffix on all file names
