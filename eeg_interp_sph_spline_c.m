...
% Find the inverse of Gx (this is where the fun really starts!)

invMethod = 'Gaussian'; % this is preferred, if possible
%invMethod = 'svd';

switch invMethod,
  
 case 'Gaussian',
  % see 'help slash', A\EYE(SIZE(A)) produces the inverse of A
 ...
 case 'svd',
  ...
  invW = inv(W);
  k = 1e-7; % for voltage (1e-5 for laplacian)
  lambda = 0;
  cutoff = k * (10^-lambda);
