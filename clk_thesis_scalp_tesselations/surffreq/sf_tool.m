function sf_tool(varargin)
% SF_TOOL	Miscellaneous MATLAB graphics tools
%		SF_TOOL(tool [, ...])
...
%   ax_kids, cb_kids, uicntl_kids, uimenu_kids

% to check for 2D plots:	if all(get(gca, 'view') == [0 90])
% to check for 3D plots:	if any(get(gca, 'view') ~= [0 90])
...
if (nargin == 0) help sf_tool; return; end
...
for k = 1:nargin
	...
	case 'all',
		...
		sf_tool  axes cmap misc shade spin zoom;
	    end
	...
	%%% menu for axis labels in figure
    	...
    	case 'axes',
    	    sf_tool noaxes;
    	    mh = sf_tool_uimenu('&Axes', 'sf_tool cbaxes', ...
	...
	%%% menu for colormap in figure
    	...
    	case 'cmap',
    	    sf_tool nocmap;
    	    mh = sf_tool_uimenu('&CMap', 'sf_tool cbcmap', ...
	...
	%%% menu for miscellaneous actions
    	...
    	case 'misc',
    	    sf_tool nomisc;
    	    mh = sf_tool_uimenu('&Misc', 'sf_tool cbmisc', ...
    	    	    	    	{'apply...', 'axis...', 'caxis...', ...
	...
	case 'cbmisc',
		...
		case 'EPSF C',
		    disp('Printing to color EPS file ''sf_out.eps''.');
		    print -noui -depsc sf_out.eps
		case 'EPSF BW',
		    disp('Printing to B/W EPS file ''sf_out.eps''.');
		    print -noui -deps sf_out.eps
	...
	case 'shade',
	    sf_tool noshade;
	    mh = sf_tool_uimenu('&Shade', 'sf_tool cbshade', ...
	...
	case 'zoom',
	    ...
	    if all(get(gca, 'view') == [0 90])
	    	zoom on;
	    	mh = sf_tool_uimenu('&Zoom', 'sf_tool cbzoom2', ...
	    	    	    	    {'on', 'xon', 'yon', 'reset', 'out', 'off'});
    	    	set(findobj(mh,'Label','on'),'Checked','on');
	    	
	    else
		mh = sf_tool_uimenu('&Zoom', 'sf_tool cbzoom3', ...
		    	       {'0.5', '0.707', '1.414', '2.0'});
	...
	case 'nospin',
	    delete(findobj(uicntl_kids, 'CallBack','sf_tool cbspin_azi'));
	    delete(findobj(uicntl_kids, 'CallBack','sf_tool cbspin_ele'));


	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	otherwise, 
	    error(['Invalid sf_tool control: ', tool]);
    end
...
function mh = sf_tool_uimenu(label,callback,entries)
