...
% $Revision: 1.5 $ $Date: 2007/02/15 05:39:48 $
...
switch electype,
  ...
  case {'cartesian','spherical1','spherical2'},
    ...
    index = find(type == 99);
    if isempty(index),
      p.elec.data.centroid = [0 0 0]; % let's assume something
    else,
      p.elec.data.centroid = [X(index) Y(index) Z(index)];
    end
    ...
    if isempty(index),
      ...
      error('there is no electrode index: 69');
    else
      elec = elec(index);
      x = X(index);
      y = Y(index);
