...
if ~strcmp(view.type,'FLAT')
    myErrorDlg('function getFlatCoords only for Flat view.');
end

if isempty(VOLUME)
    myErrorDlg('Volume window must be open.  Use "(Re-)open Volume Window" from the Window menu or type "openVolumeWindow" at the matlab prompt.');
end

pathStr=fullfile(mrSESSION.homeDir,view.subdir,'coords');
...
if ~check4File(pathStr)
    ...
    for h = 1:2
        ...
        if isempty(gLocs2d) | isempty(gLocs3d)
            ...
            % in the unfold.
            % VOLUME.coords are the gray coords that lie in the inplanes.
            % gLocs3d are the gray coords in the unfold.
            % 
            [grayCoordsTmp,gLocsIndices,coordsIndices] = ...
                intersectCols(gLocs3d,VOLUME.coords);
    ...
    % Save to file
    %
    eval(['save ',pathStr,' coords grayCoords imSize leftPath rightPath']);
end
